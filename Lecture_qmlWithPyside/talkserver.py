...
class Server(QtNetwork.QTcpServer):
    ...
    def newClient(self):
        ...
        self.clients[socket] = {}
        self.clients[socket]["name"] = u"Guest-%d" % random.randint(1, 100)

    def disconnectClient(self):
        socket = self.sender()
        self.sendAll(u"<em>%s Left here.</em>" % self.clients[socket]["name"])
    ...
    def readData(self):
        ...
        if cmd == "login":
            if self.Exist(value):
                ...
                self.send(socket, u"<em>already exist...</em>")
            else:
                name = value
                self.clients[socket]["name"] = name
            self.sendAll(u"<em>%s come in.</em>" % name)
        elif cmd == "say":
